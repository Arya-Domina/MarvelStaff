...
import com.example.marvelstaff.util.PagingRequestHelper.Request.Callback
import java.util.*
 ...
 */
    ...
    private fun prepareStatusReportLocked(): StatusReport {
        ...
        val errors = arrayOf<Throwable>(
            mRequestQueues[0].mLastError ?: Throwable("what INITIAL"),
            mRequestQueues[1].mLastError ?: Throwable("what BEFORE"),
            mRequestQueues[2].mLastError ?: Throwable("what AFTER")
        )
        return StatusReport(
    ...
    interface Request {
        ...
        class Callback internal constructor(
            ...
            fun recordFailure(@NonNull throwable: Throwable?) {
                ...
                requireNotNull(throwable) { "You must provide a throwable describing" + " the error to record the failure" }
                if (mCalled.compareAndSet(false, true)) {
    ...
    fun createStatusLiveData(): LiveData<State> {
        val liveData = MutableLiveData<State>()
        addListener(object : Listener {
            override fun onStatusChange(report: StatusReport) {
                when {
                    report.hasRunning() -> liveData.postValue(State.LOADING)
                    report.hasError() -> liveData.postValue(State.ERROR)
                    else -> liveData.postValue(State.DONE)
                }
            }
